...
class GfalCommands(CommandBase):
    ...
    @base.arg('src', type=str, help="source file")
    ...
    def _copy(self, s, d, n):
        ...
        if not self.params.verbose and sys.stdout.isatty():
            ...
            self.progress_bar = Progress('Copying %d' % n)
    ...
    def execute_rm(self):
        ...
        """
        for f in self.params.file:
            try:
                self.context.unlink(f)
            except gfal2.GError, e:
                if (e.code == errno.EISDIR or e.code == errno.ENOTEMPTY) and self.params.recursive:
                    self.__rm_dir(f)
